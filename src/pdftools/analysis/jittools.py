"""Just in time compiled tools for the analysis module."""
...
def mask_ring_median(values_array: np.ndarray, positions_array: np.ndarray, alpha: float):  # pragma: no cover
...
@jit(cache=True, nopython=True, nogil=True)
def mask_ring_mean(values_array: np.ndarray, positions_array: np.ndarray, alpha: float):  # pragma: no cover
    ...
    removals = []
    while True:
        b = np.array([item in removals for item in positions_array], dtype=boolean)
        m[b] = False
        v = values_array[m]
        if len(v) <= 1:
            break
